...
console.log(typeof +((7264 * 500) / 90).toFixed(0));
...
function booksByAuthor(dictionaryPlaceholder) {
  // Your code here:
  const newArr = [];
  for (author in dictionaryPlaceholder) {
    dictionaryPlaceholder[author].forEach((book) =>
      newArr.push({ title: book[0], pages: book[1], author: author })
    );
  }
  return newArr;
}
